...
static uint32_t SND_DEVICE_HANDSET_BACK_MIC=-1;
static uint32_t SND_DEVICE_SPEAKER_BACK_MIC=-1;
static uint32_t SND_DEVICE_NO_MIC_HEADSET_BACK_MIC=-1;
static uint32_t SND_DEVICE_HEADSET_AND_SPEAKER_BACK_MIC=-1;
// ----------------------------------------------------------------------------
   ...
   if (get_audpp_filter() == 0) {
    ...
    if (m7xsnddriverfd >= 0) {
        ...
        if (rc >= 0) {
            ...
            mInit = true;
            ALOGI("constructed (%d SND endpoints) [ found (%d SND endpoints) ]", rc, mNumSndEndpoints);//
            ...
            for (int cnt = 0; cnt < mNumSndEndpoints; cnt++, ept++) {
                ...
                ALOGI("cnt = %d ept->name = %s ept->id = %d\n", cnt, ept->name, ept->id);//
...
//                CHECK_FOR(TTY_FULL);
//                CHECK_FOR(HANDSET_BACK_MIC);
//                CHECK_FOR(SPEAKER_BACK_MIC);
//                CHECK_FOR(NO_MIC_HEADSET_BACK_MIC);
//                CHECK_FOR(HEADSET_AND_SPEAKER_BACK_MIC);
...
// Now we just reroute main speaker to whatever might give provide sound
//ALOGI("Messing around with the routing a little to find an actual output");
//SND_DEVICE_SPEAKER=SND_DEVICE_STEREO_HEADSET_AND_SPEAKER;
        }
...
*
*/
static int switch_mode( int nMode ) {
    int i2cfd = -1, rc= -1 ;
