...
/*    ALOGI("Sanity Checkpoint reached");
    if(format>1000){
        ALOGV("Oops! I've stumbled upon some weird htc-fu");
        ALOGV("found: format %d, channelMask %x", format, channelMask);
        format = 1;
        channelMask = 16;
        ALOGV("using: format %d, channelMask %x", format, channelMask);
    }*/
    ...
    input = mpClientInterface->openInput(profile->mModule->mHandle,
                                    ...
                                    &inputDesc->mChannelMask);
#endif
ALOGV("asked4: device %d, samplingRate %d, format %d, channelMask %d", inputDesc->mDevice, inputDesc->mSamplingRate, inputDesc->mFormat, inputDesc->mChannelMask);
ALOGV("got: input %d, samplingRate %d, format %d, channelMask %d", input, samplingRate, format, channelMask);
    ...
    if (input == 0 ||
        ...
        ALOGV("getInput() failed opening input: samplingRate %d, format %d, channelMask %d", samplingRate, format, channelMask);
        if (input != 0) {
